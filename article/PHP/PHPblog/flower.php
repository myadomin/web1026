...
//花朵发送验证  一旦点击了发送 就执行这段 并且下面的也不需要执行了 所以最后无论是否发送成功都exit()了
if($_GET['action'] == 'send'){
...
//获取数据 给到TO XXX
if(isset($_GET['id'])){
	//在打开他们的父窗口中 发现打开这个窗口的a链接附加了title信息 titile信息就是用户的id
	//这个新窗口然后通过相应的js打开的 打开后url就附加了 ?id=xx $_GET['id']
	//通过这个id 得到了某个指定用户的tg_id 通过这个用户的tg_id查找他的tg_username
...
<div id="message">
	...
	<form method="post" action="flower.php?action=send">
		...
		<dl>
			<dd>
				<input type="text" readonly="readonly" value="TO:<?php echo $_toUsername?>" class="text" />
				<select name="flower">
				<?php 
			...
			<dd>
				<textarea name="content">我真的真的非常非常的欣赏你，呵呵</textarea>
			</dd>
			<dd>
				验 证 码：<input type="text" name="code" class="text yzm"  /> 
