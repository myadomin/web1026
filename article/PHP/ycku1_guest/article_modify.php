...
if (!isset($_COOKIE['username'])) {
	alert_location('发帖前，必须登录','login.php');
}

//点击修改帖子后  更新数据库帖子信息  回到原来的主题帖
if($_GET['action'] == 'modify'){
	mysql_query("
			UPDATE article
			SET type='{$_POST['type']}', title='{$_POST['title']}', content='{$_POST['content']}'
			WHERE id='{$_POST['id']}'
			") or die(mysql_error());
	if(mysql_affected_rows() == 1){
		alert_location('修改成功', 'article.php?id='.$_POST['id']);
	}else{
		alert_back('修改失败');
	}
}

//点击article.php某个帖子的修改后 $_GET['id']到这里
if(isset($_GET['id'])){
	$result = mysql_query("
			SELECT type, title, content, id
			FROM article
			WHERE id='{$_GET['id']}'
			AND reid=0
			LIMIT 1
...
<title><?php echo WEBNAME?>--修改帖子</title>
<?php 
...
<div id="post">
	...
	<form method="post" name="post" action="?action=modify">
		...
		<dl>
			...
			<dd>标　　题：<input type="text" name="title" value="<?php echo $rows['title']?>" class="text" /> (*必填，2-40位)</dd>
			...
			<dd>
				...
				<textarea name="content" rows="9"><?php echo $rows['content']?></textarea>
			</dd>
