...
$result = mysql_query("
	SELECT touser, fromuser, content, date, id, flower
	FROM flower
...
$total = mysql_num_rows(mysql_query("SELECT id FROM friend WHERE touser = '{$_COOKIE['username']}'"));
...
if($_GET['action'] == 'delete' && isset($_POST['ids'])){
	...
	if(mysql_affected_rows() >= 1){
		alert_location('花朵删除成功', 'member_flower.php');
	}else{
		alert_back('花朵删除失败');
	}
}
...
<title><?php echo WEBNAME?>--花朵中心</title>
<?php 
...
<div id="member">
	...
	<div id="member_main">
		...
		<table cellspacing="1">
			<tr><th>赠送人</th><th>请求内容</th><th>时间</th><th>数量</th><th>操作</th></tr>
			<?php 
			$total_flower = 0;
			while(!!$rows = mysql_fetch_array($result)){
			$total_flower += $rows['flower'];
			?>
			<tr>
			...
			<tr>
				<td colspan="5">
					共有<strong><?php echo $total_flower ?></strong>朵
				</td>
			</tr>
			<tr>
				<td colspan="5">
